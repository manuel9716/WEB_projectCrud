...
public class TareasContext: DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)//fluent API
    {
        ...
        modelBuilder.Entity<Categoria>(categoria=> 
        {
            ...
            categoria.HasKey(p=> p.CategoriaId);// nombre de la clave

            categoria.Property(p=> p.Nombre).IsRequired().HasMaxLength(150); //especificar si es requerido

            categoria.Property(p=> p.Descripcion).IsRequired(false);

            categoria.Property(p=> p.Peso);

            categoria.HasData(categoriasInit);//permite agregar datos iniciales
        });
